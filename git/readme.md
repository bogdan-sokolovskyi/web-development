...
git config --global user.name "John Doe" # указать имя, которым будут подписаны коммиты
git config --global user.email johndoe@example.com  # указать электропочту, которая будет в описании коммитера
...
``` bash
git clone http://....git                  # клонировать удаленный репозиторий в одноименную директорию
git clone http://....git FolderName       # клонировать удаленный репозиторий в директорию «FolderName»
git clone http://....git .                # клонировать репозиторий в текущую директорию
git clone http://....git -b <branch-name> # клонировать нужную ветку
```
...
# убрать из индекса и закомитить остальные файлы
git add .
git reset -- name_file
git commit -m "..."
```
...
git commit --allow-empty -m "Trigger rebuild"  # after pushed an empty commit
```
...
git reset --hard <...hash_commit>  # ОПАСНО: отменить изменения;
git push --force
```
...
``` bash
git log --graph --all --oneline # графический вывод веток
...
git branch -r              # при клонирования репозитория, клонируется только master ветка.РЕШЕНИЕ
...
git remote add origin http://....git      # добавить удалённый репозиторий (с сокр. именем origin) с указанным URL
...
git init || npm init -y       # создаём репозиторий в этой директории
